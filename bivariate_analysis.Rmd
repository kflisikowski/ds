...
r2
```
So as we can see almost 33% of total log of incomes' variability is explained by differences in age. The rest (77%) is probably explained by other factors.
...
library(ltm)
r_bin1<-biserial.cor(bank$income, bank$def)
r_bin1
```
...
If you would like to check if there are any nonlinearities between variables, the only possibility (beside transformations and linear analysis) is to calculate "eta" coefficient and compare it with the Pearson's linear coefficient:
